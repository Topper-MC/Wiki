...
First, you add some settings to link a Top Holder to this Timed Holder, which means the Timed Holder will listen to the
...
This example will set that the Timed Holder `jump_daily` will listen to the Top Holder
`jump` ([Click here if you don't know what `jump` is](/topper/quickstart/)).

> [NOTE]
> **Link a holder from GroupTopper**
> If you want to use a holder from [**GroupTopper**](/grouptopper/), you can set the `top-type` field to `grouptopper`.
