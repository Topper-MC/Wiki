...
- [`default-value-display` in holder settings](../provider/#default-value-display)
- [`{value}` in `line`](../provider/#default-values)
- [the `value` and `top_value` queries](../../query/#types)
...
Or just the `<format>` string in [`default-value-display`](../provider/#default-value-display).
